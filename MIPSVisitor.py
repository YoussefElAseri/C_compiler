...
class MIPSVisitor:
    ...
    def visitRun(self, node: RunNode):
        ...
        for child in node.children:
            child.generateMips(self, True)

        for i in range(self.mainIndex, len(self.GlobalInit)+self.mainIndex):
            self.text.insert(i, self.GlobalInit[i-self.mainIndex])
    ...
    def visitLiteral(self, node: LiteralNode, global_var):
        ...
        if nlt == "int":
            ...
            self.treg += 1
            instructions.append(register.save(value))
        elif nlt == "float":
            register.assign(self.freg, "f")
            self.freg += 1
            instructions.append(register.save(float_to_hex(value)))
        elif nlt == "char":
            register.assign(self.treg, "t")
            self.treg += 1
            instructions.append(register.save(value))
            ...
            self.treg += 1
            if value:
                instructions.append(register.save(1))
            else:
                instructions.append(register.save(0))

        if global_var:
            self.GlobalInit.extend(instructions)
        else:
            self.text.extend(instructions)
        return register

    def visitAssignment(self, node: AssignmentNode, global_var=False):
        variable = node.left.generateMips(self, global_var)
        ...
        result = self.getValue(node.right.generateMips(self, global_var))
        ...
        if variable.type != "float" and result.type == "f":
            ...
            self.treg += 1
            self.freg -= 1
            instructions.append(convert_float_to_int(newRegister, result))
            result = newRegister
            ...
            if result.type == "t":
                self.treg -= 1
            instructions.append(convert_int_to_float(newRegister, result))
            result = newRegister

        if isinstance(variable, Local):
            instructions.append(variable.storeLocal(result, self.offset))
        elif isinstance(variable, Global):
            instructions.append(variable.storeGlobal(result))
        elif isinstance(variable, Register):
            if variable.pointerAddress:
                if variable.type == "float":
                    instructions.append(f"s.s {result}, ({variable.pointerAddress})")
                else:
                    instructions.append(f"sw {result}, ({variable.pointerAddress})")
            elif variable.arrayAddress:
                if variable.type == "float":
                    instructions.append(f"s.s {result}, ({variable.arrayAddress})")
                else:
                    instructions.append(f"sw {result}, ({variable.arrayAddress})")
    ...
    def visitUnary(self, node: UnaryNode):
        ...
        if node.operation == "&":
            ...
            if isinstance(register, Local):
                temp = register.loadAddress(self.treg, self.offset)
            ...
            if isinstance(register, Local):
                ...
                self.text.append(temp[0])
                self.text.append(f"lw {temp[1]}, ({temp[1]})")
                self.text.append(f"move {index}, {temp[1]}")
                self.text.append(f"add {index}, {index}, $sp")
    ...
    def visitArray(self, node: ArrayNode):
        ...
        # index * self.size
        self.text.append(tempRegister.save(array.arraySize))
        self.text.append(sub(index, tempRegister, index))
    ...
    def visitFunction(self, node: FunctionNode):
        ...
        if label == "main":
            ...
            self.mainIndex = len(self.text) + 1
            ...
            if isinstance(i, str):
                ...
                if i[0] == int_1:
                    ...
                    for k in range(result.arraySize):
                        ...
                        self.text.append(f"lb $a0, {(self.offset - result.offset) - (k * 4)}($sp)")
    ...
    def visitScanf(self, node: ScanfNode):
        ...
        for i in range(0, len(parsed)):
            ...
            if parsed[i][0:1] == string:
                ...
                self.data.append(f"{label}: .space 100")
                ...
                tempRegister3 = Register()
                tempRegister.assign(self.treg, "t")
                tempRegister2.assign(self.treg + 1, "t")
                for k in range(size):
                    self.text.append(f"\nla {tempRegister2}, {label}")
                    self.text.append(f"lb {tempRegister}, {k}({tempRegister2})")
                    self.text.append(tempRegister2.save((register.arraySize - k) * 4))
                    self.text.append(f"add {tempRegister2} {tempRegister2}, $sp")
                ...
                self.text.append(f"sw $v0, {self.offset - register.offset}($sp)")
